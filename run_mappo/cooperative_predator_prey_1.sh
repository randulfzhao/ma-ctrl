...
for SEED in "${SEEDS[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python runner_mappo.py \
    ...
    --wandb_group cooperative-predator-prey-mappo \
    --wandb_name "cooperative_predator_prey-mappo-gpu${GPU_ID}-seed${SEED}-${RUN_TS}" \
    --wandb_mode online \
    --wandb_tags mappo,cooperative_predator_prey,mappo_tuning,weaker_policy \
    --wandb_entity "" \
    2>&1 | tee "logs/cooperative_predator_prey_mappo_gpu${GPU_ID}_seed${SEED}_${RUN_TS}.log"
done
