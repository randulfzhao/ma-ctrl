...
for SEED in "${SEEDS[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python runner_mappo.py \
    ...
    --wandb_group cooperative-navigation-mappo \
    --wandb_name "cooperative_navigation-mappo-gpu${GPU_ID}-seed${SEED}-${RUN_TS}" \
    --wandb_mode online \
    --wandb_tags mappo,cooperative_navigation,mappo_tuning,weaker_policy \
    --wandb_entity "" \
    2>&1 | tee "logs/cooperative_navigation_mappo_gpu${GPU_ID}_seed${SEED}_${RUN_TS}.log"
done
