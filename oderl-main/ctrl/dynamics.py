...
class Dynamics(nn.Module, metaclass=ABCMeta):
    ...
    def forward_simulate(self, solver, H, s0, f, g, L, tau=None, compute_rew=True, record_actions=False):
        # starting from t=0
        T = int(H/self.env.dt)
        ...
        st, rt, at = self._forward_simulate(
            solver, ts, t0s, s0, f, g, L, tau=tau, compute_rew=compute_rew, record_actions=record_actions
        )
        return st, rt, at, torch.stack([ts[:-1]]*s0.shape[0])
    
    def forward_simulate_nonuniform_ts(self, solver, ts, s0, f, g, L, tau=None, compute_rew=True, record_actions=False):
        # all t0s are set to be zero
        [N,T] = ts.shape
        ts_norm = ts - ts[:,0:1] # all starting from 0 [[.0 .1 .4],[.1 .3 .4]] --> [[.0 .1 .4],[.0 .2 .3]]
        ts_unique = torch.unique(torch.cat([torch.zeros(1, device=ts.device), ts_norm[:,1:].reshape(-1)]), sorted=True)
        # NODE._forward_simulate keeps only len(ts)-1 points, so append one tail point
        # to preserve all original observation times after slicing.
        if ts_unique.numel() > 1:
            min_delta = torch.min(ts_unique[1:] - ts_unique[:-1]).item()
            tail_delta = max(1e-6, 0.5 * float(min_delta))
        else:
            tail_delta = max(1e-6, float(self.env.dt) * 1e-3)
        ts_ode_sorted = torch.cat([ts_unique, ts_unique[-1:] + tail_delta])
        # Use searchsorted + tolerance instead of exact float equality.
        Tidx = torch.searchsorted(ts_unique, ts_norm)
        Tidx = torch.clamp(Tidx, max=ts_unique.shape[0]-1)
        matched = ts_unique[Tidx]
        if torch.max(torch.abs(matched - ts_norm)).item() > 1e-6:
            raise RuntimeError('Nonuniform time grid mapping failed due to floating-point mismatch.')
        st, rt, at = self._forward_simulate(
            solver, ts_ode_sorted, ts[:,0], s0, f, g, L, tau=tau, compute_rew=compute_rew, record_actions=record_actions
        )
        sts = [st[:,i,Tidx[i].long()] for i in range(N)]
        rts = [rt[:,i,Tidx[i].long()] for i in range(N)]
        return torch.stack(sts,1), torch.stack(rts,1), at, ts

    @abstractmethod
    def _forward_simulate(self, solver, ts, t0s, s0, f, g, L, tau, compute_rew, record_actions):
...
class NODE(Dynamics):
    ...
    def odef(self, t, sv, f, g, t0s=None, tau=None, compute_rew=True, record_actions=False):
        ''' Input
        ...
        a  = g(s,t+t0s) # L,N,m
        if record_actions:
            # Never keep autograd graph in action history; this is for diagnostics/plots only.
            self.at[t+t0s] = a.detach()
        ds = self.ds_dt(f,s,a)
        dv = self.dv_dt(t,s,a,v,tau,compute_rew)
        return torch.cat([ds,dv],-1) # Nens,N,n+1
     
    def _forward_simulate(self, solver, ts, t0s, s0, f, g, L, tau=None, compute_rew=True, record_actions=False):
        self.at = {}
        T = len(ts)-1
        [N,n] = s0.shape # N,n
        ...
        odef = lambda t,s: self.odef(t, s, f, g, t0s, tau=tau, compute_rew=compute_rew, record_actions=record_actions)
        ...
        return st, rt, self.at if record_actions else {}
...
class PETS(Dynamics):
    ...
    def _forward_simulate(self, solver, ts, t0s, s0, f, g, L, tau=None, compute_rew=True, record_actions=False):
        ...
        return st, Vt, at if record_actions else {}
