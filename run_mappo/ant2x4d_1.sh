...
for SEED in "${SEEDS[@]}"; do
  CUDA_VISIBLE_DEVICES="${GPU_ID}" python runner_mappo.py \
    ...
    --wandb_group ant2x4d-mappo \
    --wandb_name "ant2x4d-mappo-gpu${GPU_ID}-seed${SEED}-${RUN_TS}" \
    --wandb_mode online \
    --wandb_tags mappo,ant2x4d,mappo_tuning,weaker_policy \
    --wandb_entity "" \
    2>&1 | tee "logs/ant2x4d_mappo_gpu${GPU_ID}_seed${SEED}_${RUN_TS}.log"
done
